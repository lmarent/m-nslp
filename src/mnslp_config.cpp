...
mnslp_config::setRepository(configpar_repository* cfp_rep)
{
	...
	// register the realm
	cfgpar_rep->registerRealm(mnslp_realm, "mnslp-nslp", mnslpconf_maxparno);
...
mnslp_config::registerAllPars()
{
  ...
  registerPar( new configpar<uint32>(mnslp_realm, mnslpconf_ni_msg_hop_count, "ni-msg-hop-count", "NI Message Hop Count", true, 20, "s") );  
  ...
  registerPar( new configpar<string>(mnslp_realm, mnslpconf_ms_export_config_file, "ms-export-config-file", "export configuration file name", true, "/home/luis/NSIS/nsis-ka-0.97/etc/export.xml") );
  registerPar( new configpar<string>(mnslp_realm, mnslpconf_ms_configuration_file, "ms-filter-config-file", "configuration file name", true, "/home/luis/NSIS/nsis-ka-0.97/etc/configuration.xml") );
  registerPar( new configpar<string>(mnslp_realm, mnslpconf_ms_metering_application, "ms-metering-application", "metering applicartion name", true, "netmate") );
  registerPar( new configpar<string>(mnslp_realm, mnslpconf_ms_metering_user, "ms-metering-user", "metering application user", true, "admin") );
  registerPar( new configpar<string>(mnslp_realm, mnslpconf_ms_metering_password, "ms-metering-password", "metering user password", true, "admin") );  
  registerPar( new configpar<string>(mnslp_realm, mnslpconf_ms_metering_server, "ms-metering-server", "metering http server", true, "localhost") );
  registerPar( new configpar<string>(mnslp_realm, mnslpconf_ms_metering_def_xsl, "ms-metering-def-xsl", "metering results decoding", true, "/home/luis/NSIS/nsis-ka-0.97/etc/reply2.xsl") );
  registerPar( new configpar<uint32>(mnslp_realm, mnslpconf_ms_metering_port, "ms-metering-port", "metering port", true, 12244) );
  registerPar( new configpar<string>(mnslp_realm, mnslpconf_ms_export_directory, "ms-export-directory", "export directory", true, "/tmp/") );
