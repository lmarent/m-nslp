...
mnslp_config::registerAllPars()
{
  ...
  registerPar( new configpar<string>(mnslp_realm, mnslpconf_ms_configuration_file, "filter-config", "configuration file name", true, "/home/luis/NSIS/nsis-ka-0.97/etc/configuration.xml") );
  ...
  registerPar( new configpar<string>(mnslp_realm, mnslpconf_ms_metering_user, "metering-user", "metering application user", true, "admin") );
  registerPar( new configpar<string>(mnslp_realm, mnslpconf_ms_metering_password, "metering-password", "metering user password", true, "admin") );  
  registerPar( new configpar<string>(mnslp_realm, mnslpconf_ms_metering_server, "metering-server", "metering http server", true, "localhost") );
  registerPar( new configpar<string>(mnslp_realm, mnslpconf_ms_metering_def_xsl, "xsl-for-decoding-metering-results", "metering results decoding", true, "/home/luis/NSIS/nsis-ka-0.97/etc/reply2.xsl") );
  registerPar( new configpar<string>(mnslp_realm, mnslpconf_ms_metering_port, "metering-port", "metering port", true, "12244") );
  registerPar( new configpar<string>(mnslp_realm, mnslpconf_ms_export_directory, "export-directory", "export directory", true, "/tmp/") );
