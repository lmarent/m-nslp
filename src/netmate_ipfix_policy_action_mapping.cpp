/// ----------------------------------------*- mode: C++; -*--
/// @file netmate_ipfix_policy_action_mapping.cpp
/// The netmate_ipfix_policy_action_mapping class.
/// ----------------------------------------------------------
/// $Id: netmate_ipfix_policy_action_mapping.h 2558 2015-01-16 13:49:00 amarentes $
/// $HeadURL: https://./src/netmate_ipfix_policy_action_mapping.cpp $
// ===========================================================
//                      
...
std::string netmate_ipfix_policy_action_mapping::metering_proc_name_str = "METERING_PROCEDURE";
std::string netmate_ipfix_policy_action_mapping::priority_str = "PRIORITY_APP";
std::string netmate_ipfix_policy_action_mapping::export_proc_name_str = "EXPORT_PROCEDURE";
std::string netmate_ipfix_policy_action_mapping::export_directory_str = "DIRECTORY";
std::string netmate_ipfix_policy_action_mapping::export_report_interval_str = "INTERVAL";



netmate_ipfix_policy_action_mapping::netmate_ipfix_policy_action_mapping():
...
netmate_ipfix_policy_action_mapping::netmate_ipfix_policy_action_mapping(const netmate_ipfix_policy_action_mapping &rhs):
  policy_action_mapping(rhs)
{
	if (meter_config != NULL){
		netmate_ipfix_metering_config *meter_configuration = dynamic_cast< netmate_ipfix_metering_config* >( meter_config );
...
netmate_ipfix_policy_action_mapping::copy() const
{
	netmate_ipfix_policy_action_mapping *q = NULL;
	q = new netmate_ipfix_policy_action_mapping(*this);
	return q;
}

void netmate_ipfix_policy_action_mapping::processNode(int level, xmlTextReaderPtr reader, 
						netmate_ipfix_metering_config *meter_conf) 
{
    ...
    if ((name) and (meter_conf != NULL)){
		...
		{ 
			...
			if (xmlStrEqual( name, 
				 (const xmlChar *) netmate_ipfix_policy_action_mapping::priority_str.c_str()) == 1 )
			{
				meter_conf->set_priority(atoi((processTextNode(level + 1, reader)).c_str()));
			}
			else if (xmlStrEqual( name, 
			    (const xmlChar *) netmate_ipfix_policy_action_mapping::metering_proc_name_str.c_str()) == 1 )
			{
				meter_conf->set_metering_procedure(processTextNode(level + 1, reader));
			}
			else if (xmlStrEqual( name, 
				 (const xmlChar *) netmate_ipfix_policy_action_mapping::export_proc_name_str.c_str()) == 1 )
			{
				meter_conf->set_export_procedure(processTextNode(level + 1, reader));
			}
			else if (xmlStrEqual( name, 
				 (const xmlChar *) netmate_ipfix_policy_action_mapping::export_directory_str.c_str()) == 1 )
			{
				meter_conf->set_export_directory(processTextNode(level + 1, reader));
			}
			else if (xmlStrEqual( name, 
				 (const xmlChar *) netmate_ipfix_policy_action_mapping::export_report_interval_str.c_str()) == 1 )
			{
...
int 
netmate_ipfix_policy_action_mapping::read_from_xml(int level,  xmlTextReaderPtr reader)
{
	
	int ret;
	std::string metering_app;
	const xmlChar *name, *value;
	netmate_ipfix_metering_config *meter_configuration = new netmate_ipfix_metering_config();
        ...
        while (ret == 1) 
        {
			     ...
			     and (xmlTextReaderNodeType(reader) == XML_READER_TYPE_ELEMENT ) )
			{
				...
				if (xmlTextReaderDepth(reader) < level){
					...
					meter_config = meter_configuration; 
					return ret;
				}
				else{
					ret = xmlTextReaderRead(reader);
...
std::string 
netmate_ipfix_policy_action_mapping::get_key() const
{
	if (meter_config != NULL){
		netmate_ipfix_metering_config *meter_configuration = dynamic_cast< netmate_ipfix_metering_config* >( meter_config );
