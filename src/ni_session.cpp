...
/**
 * Build a MNSLP Configure message based on the session's state.
 *
...
void ni_session::setup_session(api_configure_event *e) {
...
ni_session::state_t ni_session::handle_state_close(dispatcher *d, event *evt) 
{
	...
	if ( is_api_configure(evt) ) {
		...
		set_last_configure_message( build_configure_message(e) );
 ...
 */
ni_session::state_t ni_session::handle_state_pending_participating(
 ...
 */
ni_session::state_t ni_session::handle_state_pending_forward(
		dispatcher *d, event *evt) {
	...
	if ( is_mnslp_response(evt, get_last_configure_message()) ) {
		...
		/*
		 * Discard if this is no RESPONSE to our last configure.
		 */
		mnslp_configure *c = get_last_configure_message()->get_mnslp_configure();
		if ( ! resp->is_response_to(c) ) {
		...
		if ( resp->is_success() ) {
			...
			return STATE_METERING_FORW; // Session established successfully
		}
		else {
 ...
 */
ni_session::state_t ni_session::handle_state_metering_participating(
		dispatcher *d, event *evt) {
	 ...
	 */
	if ( is_mnslp_response(evt, get_last_refresh_message() ) ) {
		msg_event *e = dynamic_cast<msg_event *>(evt);
		mnslp_response *resp = e->get_response();
	 ...
	 */
		...
		// TODO AM: change this code, we have to send a refresh message.
		// dispatcher will delete
		d->send_message( build_refresh_message() );
 ...
 */
ni_session::state_t ni_session::handle_state_metering_forward(
		dispatcher *d, event *evt) {

	using namespace mnslp::msg;

	/*
	 * A mnslp response message arrived in response to our Refresh message.
	 */
	if ( is_mnslp_response(evt, get_last_refresh_message() ) ) {
		msg_event *e = dynamic_cast<msg_event *>(evt);
		mnslp_response *resp = e->get_response();
	 ...
	 */
		...
		// Build a new REFRESH message store a copy for refreshing.
		set_last_refresh_message( build_refresh_message() );
...
void ni_session::process_event(dispatcher *d, event *evt) {
	...
	switch ( get_state() ) {
		...
		case ni_session::STATE_PENDING_FORW:
			state = handle_state_pending_forward(d, evt);
			break;

		case ni_session::STATE_PENDING_PART:
			state = handle_state_pending_participating(d, evt);
			break;

		case ni_session::STATE_METERING_FORW:
			state = handle_state_metering_forward(d, evt);
			break;

		case ni_session::STATE_METERING_PART:
